...
public class PartyConverter {
    ...
    // List<String> tuttiRank = p.getAdventurers().stream().map(a ->
    // a.getAdventurer_rank()).toList();
    ...
    // List<Integer> ranks = p.getAdventurers().stream().map(a ->
    // rankToNumber.get(a.getAdventurer_rank()))
    ...
    // int average = ranks.stream().reduce(0, (partial, el) -> partial + el) /
    // ranks.size();
    // int rank = (average + low) / 2;
    // String mediaRank = rankToNumber.entrySet().stream().filter(e -> e.getValue()
    // == rank).findFirst().get()
    // .getKey();
